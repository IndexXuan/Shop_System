...
function Mlogs(click){//mobile onSubmit验证函数 防非法提交
	...
	,click)
		   check(Mcardpass,Lcp,"初始为6位",function(val){
		if(val.length >= 6 && val.length <= 20){
			return true;
		}
		else{
   ...
   //兼容移动端 导航条直接变为顶部固定
    if( $('#footer').width() <= 650 ){
	 ...
	 //小屏幕视图改变
	    $('#addF').hide();
		$('#loga').hide();
		$('.spot').hide();//mobile hide pics
		$('#spotbtn').hide();
		$('#mobilelog').show();	
		var nav = $('#navbardiv');
	    nav.addClass('navbar-fixed-top'); 
		nav.css('top','0'); //为了清除本身的浮动，以后有好的办法再改		
        $('#footp').width('520px'); 
