<?php /* Smarty version Smarty-3.1.14, created on 2014-06-12 14:44:16
         compiled from "index.html" */ ?>
<?php /*%%SmartyHeaderCode:664253946342c96ab6-64136113%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  ...
  'nocache_hash' => '664253946342c96ab6-64136113',
  'function' => 
  array (
  ),
  'version' => 'Smarty-3.1.14',
  'unifunc' => 'content_53946342d2e4c4_41700790',
  'variables' => 
  array (
    'khzl' => 0,
    'vo' => 0,
...
<?php if ($_valid && !is_callable('content_53946342d2e4c4_41700790')) {function content_53946342d2e4c4_41700790($_smarty_tpl) {?><!DOCTYPE html>
    ...
    <!-- /.Modal end-->
        ...
        <aside class="rightbox area area_block" style="*width:45%;height:220px;font-size:14px;letter-spacing:0;"><!---抵消 child_join 效果-->     
            ...
            <div class="gap20"></div>   
                <ul class="inline">
                    <li>
                      <button class="btn btn-success"><?php echo $_SESSION['username'];?>
 <?php ob_start();?><?php echo $_SESSION['permission'];?>
<?php $_tmp2=ob_get_clean();?><?php if ($_tmp2>2){?>总管理员<?php }?></button><li>
...
.
            ...
            <div class="tab-pane" id="three"><!-- 库存管理 -->
                <input type="search" id="search_kc">
...
function draw() {
    ...
    $.get('caiwu_info.php?caiwu_type=' + caiwu_type + '&starttime=' + starttime + '&endtime=' + endtime, function(data) {
        //alert(data);
        var Gdata = [];    
        var labels = [];
        var data1 = [];
...
$('#caigou_sub').click(function(e) {
    ...
    $.post('ajaxHandler.php', postinfo, function(data, status) {
        if(status == 'success') {
            $('.reset').click();
            $('#postop input, textarea, #postop select ,#postop p').show();
            $('#postop').hide(300);
...
$('#new_sub').click(function(e) {
    ...
    // 现在可以使用$.get、$.post、$.ajax等来提交数据
    $.post('addUser.php', postinfo, function(data, status) {           
        if(status == 'success') {
            $('.reset').click();
            $('#postuser').hide(300);
        } else {
...
$('#user_modify_btn').click(function(e) {
    ...
    // 现在可以使用$.get、$.post、$.ajax等来提交数据
    $.post('modifyUser.php', postinfo, function(data, status) {
        if(status == 'success') {
            $('.reset').click();
            $('#postmodify').hide(300);
        }
...
$('#log_btn').click(function(e) {
    ...
    // 现在可以使用$.get、$.post、$.ajax等来提交数据
    $.post('sys_log.php', postinfo, function(data, status) {
        if(status == 'success') {
            $('.reset').click();
            $('#postlog').hide(300);
        }
...
$('#line_btn').click(function(e) {
    ...
    // 现在可以使用$.get、$.post、$.ajax等来提交数据
    $.post('set_line.php', postinfo, function(data, status) {
        if(status == 'success') {
            $('.reset').click();
            $('#postline').hide(300);
        }
